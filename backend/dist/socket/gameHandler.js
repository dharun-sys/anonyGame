...
exports.reactToAnswer = reactToAnswer;
exports.voteAnswer = voteAnswer;
exports.restartGame = restartGame;
exports.startMemeChain = startMemeChain;
exports.submitMemeChainEntry = submitMemeChainEntry;
exports.voteMemeChain = voteMemeChain;
const roomHandler_1 = require("./roomHandler");
const questions_1 = require("../data/questions");
const missions_1 = require("../data/missions");
const secretPhrases_1 = require("../data/secretPhrases");
/* ──────── helpers ──────── */
function shuffle(a) {
...
function getRevealTime(playerCount) { return playerCount < 5 ? 30 : 60; }
function getVoteTime(playerCount) { return playerCount < 5 ? 30 : 60; }
...
function clearAll(roomId) {
    for (const s of [':t', ':cd', ':vt', ':vcd'])
        clearT(roomId + s);
}
/* ================================================================
   ROUND GENERATION
   ================================================================ */
function genRounds(players) {
    const ids = players.map(p => p.id);
    const n = ids.length;
    const rounds = [];
    const byType = (t) => questions_1.QUESTIONS.filter(q => q.type === t);
    /* ── per-player rounds (2 each) ── */
    const targets = shuffle([...ids, ...ids]);
    const stdTypes = ['standard', 'caption', 'future', 'excuse', 'prediction', 'compliment', 'secret_word'];
    ...
    if (spQ.length) {
        ...
        rounds.push({ type: 'secret_phrase', targetPlayerId: tid, questionText: spQ[0].template.replace(/{player}/g, p.displayName), answers: [], votes: {}, phase: 'answering', secretPhrasePlayerId: pick(nonT), secretPhrase: pick(secretPhrases_1.SECRET_PHRASES) });
    }
    /* ── 1 compliment ── */
...
function startGame(io, roomId) {
    ...
    for (const p of room.players) {
        p.score = 0;
        p.achievements = [];
        p.missionCompleted = false;
        p.roundWins = 0;
        p.totalReactionsReceived = 0;
        p.answersSubmitted = 0;
        p.consecutiveWins = 0;
    }
    // assign missions
    const ms = shuffle(missions_1.MISSIONS.slice());
    ...
    // send private info
    for (const p of room.players) {
        const m = missions_1.MISSIONS.find(x => x.id === p.secretMissionId);
        if (m)
            io.to(p.id).emit('secret_mission', { mission: m.description });
    }
...
/* ================================================================
   NEW ROUND
   ================================================================ */
function emitNewRound(io, roomId) {
    const room = (0, roomHandler_1.getRoom)(roomId);
    if (!room)
        return;
    ...
    // answer countdown
    let rem = ANSWER_TIME;
    const cd = setInterval(() => { rem--; io.to(roomId).emit('round_timer', { timeLeft: rem }); if (rem <= 0)
...
/* ================================================================
   SUBMIT ANSWER
   ================================================================ */
function submitAnswer(io, roomId, answer) {
    const room = (0, roomHandler_1.getRoom)(roomId);
    if (!room)
        return;
    const r = room.rounds[room.currentRoundIndex];
    ...
    // who can answer?
    if (r.type === 'lie_detection') {
        if (r.targetPlayerId === answer.playerId) {
            answer.isRealAnswer = true;
            r.lieDetectionRealAnswerId = answer.answerId;
        }
    }
    else {
        if (r.targetPlayerId === answer.playerId)
            return;
        if (r.targetPlayerId2 === answer.playerId)
            return;
    }
    if (r.answers.find(a => a.playerId === answer.playerId))
        return;
    answer.reactions = {};
...
/* ================================================================
   REACT TO ANSWER
   ================================================================ */
function reactToAnswer(io, roomId, playerId, answerId, emoji) {
    const room = (0, roomHandler_1.getRoom)(roomId);
    if (!room)
        return;
    const r = room.rounds[room.currentRoundIndex];
...
function startVoteTimer(io, roomId, duration) {
    let rem = duration || getVoteTime(4);
    const cd = setInterval(() => { rem--; io.to(roomId).emit('vote_timer', { timeLeft: rem }); if (rem <= 0)
        clearInterval(cd); }, 1000);
...
/* ================================================================
   VOTE
   ================================================================ */
function voteAnswer(io, roomId, voterId, value) {
    const room = (0, roomHandler_1.getRoom)(roomId);
    if (!room)
        return;
    const r = room.rounds[room.currentRoundIndex];
    ...
    if (r.type === 'secret_phrase') {
        ...
        if (voterId === r.targetPlayerId)
            return;
        r.votes[voterId] = value;
        if (Object.keys(r.votes).length >= room.players.length - 1) {
            clearAll(roomId);
            processRoundEnd(io, roomId);
        }
    }
    else if (r.type === 'double') {
        if (voterId !== r.targetPlayerId && voterId !== r.targetPlayerId2)
            return;
        r.votes[voterId] = value;
        if (r.votes[r.targetPlayerId] && r.votes[r.targetPlayerId2]) {
            clearAll(roomId);
            processRoundEnd(io, roomId);
        }
    }
...
/* ================================================================
   PROCESS ROUND END
   ================================================================ */
function processRoundEnd(io, roomId) {
    clearAll(roomId);
    const room = (0, roomHandler_1.getRoom)(roomId);
    if (!room)
        return;
    ...
    if (!r)
        return;
    r.phase = 'done';
    const res = { type: r.type, roundIndex: room.currentRoundIndex };
    /* ── scoring by type ── */
    if (r.type === 'lie_detection') {
        const correct = [];
        for (const [vid, aid] of Object.entries(r.votes)) {
            if (aid === r.lieDetectionRealAnswerId) {
                correct.push(vid);
        ...
        if (correct.length === 0) {
            const t = room.players.find(p => p.id === r.targetPlayerId);
            if (t)
                t.score += 2;
            res.targetFooledEveryone = true;
        }
        ...
        if (tv) {
            ...
            if (a) {
                ...
                res.winningAnswerId = tv;
                res.winningText = a.text;
            }
        }
    ...
    if (r.crowdFavoriteAnswerId) {
        ...
        if (ca) {
            const au = room.players.find(p => p.id === ca.playerId);
            if (au) {
                au.score++;
                res.crowdFavorite = { text: ca.text };
            }
        }
    }
    // reaction stats
...
function checkAchievements(room) {
    for (const p of room.players) {
        if (p.roundWins >= 3 && !p.achievements.includes('roast_king'))
            p.achievements.push('roast_king');
        // chaos agent: most reactions on single answer in latest round
        const r = room.rounds[room.currentRoundIndex];
        if (r) {
            const pa = r.answers.find((a) => a.playerId === p.id);
            if (pa) {
                const tot = Object.values(pa.reactions || {}).reduce((s, arr) => s + arr.length, 0);
                if (tot >= 5 && !p.achievements.includes('chaos_agent'))
                    p.achievements.push('chaos_agent');
            }
        }
    }
}
    ...
    for (const p of room.players) {
        ...
        switch (m.check) {
            case 'shortest_answer_win': {
                ...
                if (pa && vote && r.answers.find((a) => a.answerId === vote)?.playerId === p.id) {
                    const shortest = r.answers.reduce((m2, a) => a.text.length < m2.text.length ? a : m2, r.answers[0]);
                    if (shortest.playerId === p.id)
                        done = true;
                }
                break;
            }
            case 'three_reactions': {
                const pa = r.answers.find((a) => a.playerId === p.id);
                if (pa && Object.values(pa.reactions || {}).reduce((s, arr) => s + arr.length, 0) >= 3)
                    done = true;
                break;
            }
            case 'two_wins_in_row': {
                if (p.consecutiveWins >= 2)
                    done = true;
                break;
            }
            case 'win_first_round': {
                if (room.currentRoundIndex === 0 && p.roundWins >= 1)
                    done = true;
                break;
            }
            case 'crowd_favorite': {
...
function endGame(io, roomId) {
    ...
    for (const p of room.players) {
        io.to(p.id).emit('your_achievements', {
            ...
            missionDescription: missions_1.MISSIONS.find(m => m.id === p.secretMissionId)?.description,
        });
    }
    io.to(roomId).emit('leaderboard', { leaderboard, insights });
    room.gameStarted = false;
}
function computeInsights(room) {
    const ps = room.players;
    const mysterious = [...ps].sort((a, b) => a.answersSubmitted - b.answersSubmitted)[0];
    const predictable = [...ps].sort((a, b) => b.roundWins - a.roundWins)[0];
    const funniest = [...ps].sort((a, b) => b.totalReactionsReceived - a.totalReactionsReceived)[0];
    const chaotic = [...ps].sort((a, b) => b.achievements.length - a.achievements.length)[0];
    return {
...
/* ================================================================
   MEME CHAIN MODE
   ================================================================ */
function startMemeChain(io, roomId) {
    const room = (0, roomHandler_1.getRoom)(roomId);
    if (!room || room.gameStarted)
        return;
    if (room.players.length < 4) {
        io.to(roomId).emit('game_error', { message: 'Need at least 4 players' });
        return;
    }
    room.gameStarted = true;
    ...
    const prompt = pick(questions_1.MEME_CHAIN_PROMPTS).replace(/{player}/g, first.displayName);
...
function submitMemeChainEntry(io, roomId, playerId, text) {
    const room = (0, roomHandler_1.getRoom)(roomId);
    if (!room || !room.memeChain)
        return;
    const mc = room.memeChain;
    if (mc.playerOrder[mc.currentPlayerIndex] !== playerId)
        return;
...
function voteMemeChain(io, roomId, voterId, entryIndex) {
    const room = (0, roomHandler_1.getRoom)(roomId);
    if (!room || !room.memeChain)
        return;
    const mc = room.memeChain;
    if (mc.votes[voterId] !== undefined)
        return;
    mc.votes[voterId] = entryIndex;
    if (Object.keys(mc.votes).length >= room.players.length) {
        // tally
        const counts = {};
        for (const idx of Object.values(mc.votes))
